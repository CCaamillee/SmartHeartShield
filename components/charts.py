...
from components.i18n import ui_text
...
def cohort_distribution_chart(metrics: dict) -> go.Figure:
    ...
    fig = go.Figure(
        go.Pie(
            labels=["Cardiac Rupture Cohort", "Non-rupture Cohort"],
            values=counts,
            hole=0.68,
            ...
            hovertemplate="%{label}: %{value:,} encounters<extra></extra>",
        )
    )
    fig.update_layout(
        **PLOT_LAYOUT,
        ...
        annotations=[
            dict(
                text=f"<b>{metrics['total']:,}</b><br><span style='font-size:11px'>encounters</span>",
                x=0.5,
                y=0.5,
                showarrow=False,
                font_size=17,
...
def cohort_feature_chart(metrics: dict) -> go.Figure:
    ...
    labels = ["Median Age (years)", "Median LVEF (%)"]
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(name="Cardiac Rupture Cohort", x=labels, y=break_values, marker_color=RISK_COLORS["HIGH"]))
    fig.add_trace(go.Bar(name="Non-rupture Cohort", x=labels, y=nonbreak_values, marker_color="#4E91D8"))
    fig.update_layout(
        **PLOT_LAYOUT,
        height=280,
        barmode="group",
...
def _distribution_bar_chart(
    ...
    *,
    ...
    fig = go.Figure(
        go.Bar(
            x=[ui_text(row["label"]) for row in rows],
...
def prediction_review_chart(rows: list[dict]) -> go.Figure:
    ...
    fig = go.Figure(
        go.Pie(
            labels=[ui_text(row["label"]) for row in rows],
            values=counts,
            hole=0.64,
    ...
    fig.update_layout(
        **PLOT_LAYOUT,
        ...
        annotations=[
            dict(
                text=f"<b>{total}</b><br><span style='font-size:11px'>assessed</span>",
                x=0.5,
                y=0.5,
                showarrow=False,
                font_size=17,
...
def signal_curve(snapshots: list[dict], current_index: int | None = None, height: int = 320) -> go.Figure:
    ...
    fig = go.Figure(
        go.Scatter(
            ...
            hovertemplate="%{x}<br>Cumulative review signals %{y}<br>%{text}<extra></extra>",
        )
    )
    fig.update_layout(
        **PLOT_LAYOUT,
...
def vital_snapshot_chart(rows: list[dict]) -> go.Figure:
    if not rows:
        fig = go.Figure()
        fig.add_annotation(text="No structured vital signs for this patient", x=0.5, y=0.5, showarrow=False)
