...
from components.i18n import display_value, source_label as translated_source_label, ui_text


TOOL_LABELS = {
    "extract_clinical_features": "Organize Clinical Records",
    "get_patient_timeline": "Review Clinical Timeline",
    "calculate_risk": "Run Risk Prediction",
    "knowledge_search": "Review Medical Knowledge",
}

THINKING_SECTION_LABELS = {
    "关键依据": "Key Evidence",
    "关键证据": "Key Evidence",
    "支持依据": "Supporting Evidence",
    "反向依据": "Counterevidence",
...
def thinking_sections(thinking: str) -> list[tuple[str, str]]:
    ...
    if not matches:
        return [("Model Analysis", clean)]

    sections: list[tuple[str, str]] = []
    prefix = clean[: matches[0].start()].strip(" \n。；：:")
    if prefix:
        sections.append(("Model Analysis", prefix))
...
def _risk_result_html(fields: dict[str, Any], answer: str) -> str:
    ...
    if not (rupture or urgency or evidence):
        return (
            ...
            "<div class='xd-react-answer-label'>Prediction Result</div>"
            f"<div class='xd-react-answer-text'>{html.escape(display_value(answer)).replace(chr(10), '<br>')}</div>"
            "</div>"
        ) if answer else ""
    ...
    if rupture:
        result_items.append(
            ...
            "<div class='xd-react-result-name'>Rupture Within 14 Days</div>"
            f"<div class='xd-react-result-value {rupture_class}'>{html.escape(ui_text(rupture))}</div>"
            "</div>"
        )
    if urgency:
        result_items.append(
            "<div class='xd-react-result-item'>"
            "<div class='xd-react-result-name'>Current Acuity</div>"
            f"<div class='xd-react-result-value {urgency_class}'>{html.escape(ui_text(urgency))}</div>"
            "</div>"
        )
    evidence_html = (
        "<div class='xd-react-answer-label'>Key Evidence</div>"
        f"<div class='xd-react-answer-text'>{html.escape(display_value(evidence)).replace(chr(10), '<br>')}</div>"
        if evidence else ""
    )
    return (
        "<div class='xd-react-answer'>"
...
def timeline_html(steps: list[dict[str, Any]]) -> str:
    rows: list[str] = []
    for step in steps:
        title = html.escape(display_value(step.get("title") or "Review Records"))
        ...
        detail = display_value(step.get("detail") or "").strip()
...
def source_label(source: object) -> str:
    ...
    if value.startswith("阿里百炼模型") or any(
        ...
        return "Medical Knowledge Summary"
    if "心脏破裂" in value or "cardiac-rupture" in lowered or "gpu" in lowered:
        return "Cardiac Rupture Prediction Model"
    if value == "结构化时间字段解析":
        return "Structured Timestamp Parsing"
    if value == "预测截点前结构化字段":
        return "Structured Fields Before the Prediction Cutoff"
    if value.startswith("本地审核知识条目"):
        return value.replace("本地审核知识条目", "Locally Reviewed Knowledge Entry")
    return translated_source_label(value)
...
def source_strip_html(sources: list[object]) -> str:
    ...
    if not labels:
        return ""
    chips = "".join(
        f"<span class='xd-react-source'>Source: {html.escape(label)}</span>" for label in labels
    )
...
def restored_timeline_steps(message: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in message.get("react_steps") or []:
        ...
        if phase == "Reason":
            steps.append({
                "title": str(item.get("title") or "Clarify the Question"),
                "detail": str(item.get("summary") or ""),
            })
        elif phase == "Observation":
            tool = str(item.get("tool") or "")
            step = {
                "title": TOOL_LABELS.get(tool, "Review Relevant Records"),
                "detail": str(item.get("summary") or ""),
            ...
            steps.append({
                "title": str(item.get("title") or "Prepare the Answer"),
                "detail": str(item.get("summary") or ""),
            })
    return steps
