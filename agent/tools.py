...
REACT_DATABASE_SOURCE = "patient_data.db（ReAct专用只读样本）"


def _uses_react_database(patient_id: str) -> bool:
    return str(patient_id or "").strip().upper().startswith("XD-")


def _patient_detail(patient_id: str) -> tuple[dict[str, Any], str]:
    if _uses_react_database(patient_id):
        from services.react_data import get_encounter_detail

        return get_encounter_detail(patient_id), REACT_DATABASE_SOURCE
    from services.workbook_data import get_encounter_detail

    return get_encounter_detail(patient_id), WORKBOOK_SOURCE
...
def get_patient_timeline(patient_id: str) -> dict[str, Any]:
    ...
    detail, source = _patient_detail(patient_id)
...
def extract_clinical_features(patient_id: str, focus: str = "") -> dict[str, Any]:
    if patient_id == COHORT_SCOPE_ID:
        from services.workbook_data import get_metrics
    ...
    unavailable = [
        {
            ...
            "detail": "当前结构化数据本身没有实时模型输出；风险结果只能来自已配置的预测模型。",
            "source": "字段可用性检查",
        }
    ]
    unavailable.extend(
    ...
    untimed_evidence = [
        {
            "name": str(item.get("evidence_id") or "临床事实"),
            "value": str(item.get("text") or "暂无记录")[:1200],
            "source": "预测截点前资料（具体时间未可靠对齐）",
        }
        for item in clinical_context.get("untimed_evidence", [])[:24]
    ]
    grounded_evidence = [*window_evidence, *untimed_evidence]
    return {
        "patient_id": patient_id,
        ...
        "important_features": grounded_evidence[:24] or _feature_payload(all_records, 24),
        "features": {
            "supporting": grounded_evidence[:12],
            "counter": [],
            "missing": unavailable,
        },
        "data_quality": {
