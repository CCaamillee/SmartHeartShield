...
from services.workbook_data import (
    get_encounter_source_record as get_workbook_source_record,
    get_source_name as get_workbook_source_name,
    workbook_signature as get_workbook_signature,
)
...
def _uses_react_database(encounter_key: str) -> bool:
    return str(encounter_key or "").strip().upper().startswith("XD-")


def _source_record(encounter_key: str) -> dict[str, Any]:
    if _uses_react_database(encounter_key):
        from services.react_data import get_source_record

        return get_source_record(encounter_key)
    return get_workbook_source_record(encounter_key)


def _source_name(encounter_key: str) -> str:
    if _uses_react_database(encounter_key):
        from services.react_data import get_source_name

        return get_source_name()
    return get_workbook_source_name()


def _source_signature(encounter_key: str) -> tuple[Any, ...]:
    if _uses_react_database(encounter_key):
        from services.react_data import get_source_signature

        return get_source_signature()
    return get_workbook_signature()
...
def _build_context(
    ...
    *,
    ...
    row = _source_record(encounter_key)
    source_name = _source_name(encounter_key)
    ...
    return {
        ...
        "sources": [source_name, "确定性时间窗与字段对齐规则"],
    }
...
def get_patient_clinical_context(
    ...
    *,
    ...
    if _uses_react_database(encounter_key):
        from services.react_data import get_prediction_context
        ...
        if progress_callback is not None:
            try:
                ...
                pass
        result = get_prediction_context(
            encounter_key,
            use_llm_compression=enabled,
            compressor_client=compressor_client,
        )
        if progress_callback is not None:
            try:
                progress_callback(
                    {"type": "context_progress", "label": "已完成症状与检查资料整理"}
                )
            except Exception:
                pass
        return result
    ...
    if as_of_time is not None or compressor_client is not None or not use_llm_compression:
        ...
        result = deepcopy(
            _cached_context(
                encounter_key,
                _source_signature(encounter_key),
                get_context_model(),
                enabled,
            )
        )
