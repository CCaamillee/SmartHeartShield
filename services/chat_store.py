...
def _normalize_encounter_key(encounter_key: object) -> str:
    if encounter_key is None:
        raise ValueError("encounter_key 不能为空")
    normalized = str(encounter_key).strip()
    if not normalized:
        raise ValueError("encounter_key 不能为空")
    return normalized
...
def load_encounter_chat(
    ...
    *,
    ...
    normalized_key = _normalize_encounter_key(encounter_key)
...
def load_chat_feed(
    *,
    ...
    """Load the persisted UI transcript across encounters in message order."""
    with closing(_connect(database_path)) as connection:
        rows = connection.execute(
            """
            SELECT encounter_key, payload
            FROM chat_messages
            ORDER BY id ASC
            """
        ).fetchall()
    ...
    messages: list[dict[str, Any]] = []
    for encounter_key, payload in rows:
        try:
            message = json.loads(payload)
        except (TypeError, json.JSONDecodeError):
            continue
        if isinstance(message, dict) and message.get("role") in {"user", "assistant"}:
            messages.append(
                {
                    **message,
                    "encounter_key": _normalize_encounter_key(encounter_key),
                }
            )
    return messages
    ...
    *,
    ...
    with closing(_connect(database_path)) as connection:
        ...
        connection.execute(
            ...
            """,
            (normalized_key, normalized_key, bounded_max),
        )
        connection.commit()
...
def clear_chat_feed(*, database_path: Path | None = None) -> None:
    """Delete the complete persisted transcript after an explicit UI action."""
    with closing(_connect(database_path)) as connection:
